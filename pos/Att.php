        ...
        if ($clockType === 'in') {
            ...
            if ($attendance && $attendance['sign_in_time']) {
                if ($attendance['sign_out_time']) {
                    ...
                    // Calculate total hours using timestamp difference
                    $totalHours = ($signOutTime->getTimestamp() - $signInTime->getTimestamp()) / 3600;
                    
                    // Combine notes properly
                    $combinedNotes = trim($attendance['notes'] ?? '');
                    if ($notes) {
                    ...
                    if ($updated !== false) {
                        $success = 'Sign-out recorded successfully. Total hours: ' . round($totalHours, 2);
                    } else {
        ...
        if (empty($qidNumber)) {
            ...
            if (!$staff) {
                ...
                if (!$attendance || !$attendance['sign_in_time']) {
                    ...
                    if ($updated) {
                        $success = $staff['name'] . ' signed in successfully at ' . date('H:i', strtotime($now));
                    } else {
                        $error = 'Failed to sign in ' . $staff['name'];
                    }
                    ...
                    if ($updated !== false) {
                        $success = $staff['name'] . ' signed out successfully at ' . date('H:i', strtotime($now)) . 
                                 '. Total hours: ' . round($totalHours, 2);
                    } else {
                        $error = 'Failed to sign out ' . $staff['name'];
                    }
                } else {
...
<body class="bg-gray-50">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="qid_number" id="qid-scanner" 
                                       class="w-full p-3 text-lg font-mono text-center border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-base"
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="staff_id" required 
                                        class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary text-base">
                                    <option value="">Choose staff member</option>
                                    <?php foreach ($activeStaff as $staff): ?>
                                    <option value="<?php echo $staff['id']; ?>">
                                        <?php echo htmlspecialchars($staff['name']); ?> - <?php echo htmlspecialchars(User::getRoleName($staff['role'])); ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows="2" 
                                          class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary resize-none text-base"
                                    ...
                                    <tr class="border-t border-gray-200">
                                        <td class="py-3">
                                            <div class="flex items-center">
                                                ...
                                                <div>
                                                    <div class="font-medium text-gray-900"><?php echo htmlspecialchars($attendance['name']); ?></div>
                                                    <div class="text-xs text-gray-500">QID: <?php echo htmlspecialchars($attendance['qid_number'] ?? 'N/A'); ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-3 text-gray-600">
                                            <span class="px-2 py-1 text-xs bg-gray-100 text-gray-800 rounded-full">
                                                <?php echo htmlspecialchars(User::getRoleName($attendance['role'])); ?>
                                            </span>
                                        </td>
                                        <td class="py-3 text-gray-600">
                                        ...
                                        <td class="py-3 text-gray-600">
                                            <?php echo $attendance['total_hours'] ? number_format($attendance['total_hours'], 1) . 'h' : '-'; ?>
                                        </td>
                                        <td class="py-3">
                                        ...
                                        <td class="py-3 text-gray-600">
                                            <?php echo $attendance['notes'] ? (strlen($attendance['notes']) > 20 ? substr($attendance['notes'], 0, 20) . '...' : $attendance['notes']) : '-'; ?>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="text-sm">
                                    ...
                                    <tr class="border-t border-gray-200">
                                        ...
                                        <td class="py-3 font-medium text-gray-900"><?php echo htmlspecialchars($record['name']); ?></td>
                                        <td class="py-3 text-gray-600"><?php echo htmlspecialchars(User::getRoleName($record['role'])); ?></td>
                                        <td class="py-3 text-gray-600">
    ...
    <script>
        ...
        // Clear QID input after submission and refocus
        document.addEventListener('DOMContentLoaded', function() {
            const qidInput = document.getElementById('qid-scanner');
            if (qidInput) {
                // Clear the input if there was a successful operation
                <?php if ($success): ?>
                qidInput.value = '';
                setTimeout(() => qidInput.focus(), 1000);
                <?php else: ?>
                qidInput.focus();
                <?php endif; ?>
            }
